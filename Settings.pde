  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<settings.data.length;i++) {
        if (settings.data[i].equals("Level Start")) levelStart = setInt(settings.data[i+1]);
        if (settings.data[i].equals("Level End")) levelEnd = setInt(settings.data[i+1]);
        if (settings.data[i].equals("Experience Points per Level")) expPoints = setInt(settings.data[i+1]);
        if (settings.data[i].equals("Multiply Points")) multPoints = setFloat(settings.data[i+1]);
        if (settings.data[i].equals("Add Points")) addPoints = setInt(settings.data[i+1]);
       }
    } 
    catch(Exception e) {
